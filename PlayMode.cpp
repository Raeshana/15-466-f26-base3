...
GLuint graveyard_meshes_for_lit_color_texture_program = 0;
Load< MeshBuffer > graveyard_meshes(LoadTagDefault, []() -> MeshBuffer const * {
	MeshBuffer const *ret = new MeshBuffer(data_path("graveyard.pnct"));
	graveyard_meshes_for_lit_color_texture_program = ret->make_vao_for_program(lit_color_texture_program->program);
	return ret;
});

Load< Scene > graveyard_scene(LoadTagDefault, []() -> Scene const * {
	return new Scene(data_path("graveyard.scene"), [&](Scene &scene, Scene::Transform *transform, std::string const &mesh_name){
		Mesh const &mesh = graveyard_meshes->lookup(mesh_name);
		...
		drawable.pipeline.vao = graveyard_meshes_for_lit_color_texture_program;
